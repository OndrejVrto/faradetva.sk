...
class RoleController extends Controller {
    ...
    public function store(RoleRequest $request): RedirectResponse {
        ...
        toastr()->success((string) __('app.role.store', ['name'=> $role->name]));
    ...
    public function update(RoleRequest $request, Role $role): RedirectResponse {
        ...
        toastr()->success((string) __('app.role.update', ['name'=> $role->name]));
        return to_route('roles.index');
    }

    public function destroy(Role $role): RedirectResponse {
        if ($role->id == 1) {
            toastr()->error((string) __('app.role.delete-error', ['name'=> $role->name]));
        } else {
            $role->delete();
            toastr()->success((string) __('app.role.delete', ['name'=> $role->name]));
        }
