    ...
    <figcaption rel="license" {{ $attributes->merge(['class' => 'source']) }}>
        @if($dimensionSource == "small" OR $dimensionSource == "medium" OR $dimensionSource == "full")
            @isset($source)
                zdroj:
                @isset($source_url)
                    <a href="{{ url($source_url) }}" target="_blank" rel="noopener noreferrer external" class="link-template">
                @endisset
        ...
        @if($dimensionSource == "medium" OR $dimensionSource == "full")
            @isset($author)
                autor:
                @isset($author_url)
                    <a href="{{ url($author_url) }}" target="_blank" rel="noopener noreferrer external" class="link-template">
                @endisset
        ...
        @if($dimensionSource == "full")
            @isset($license)
                licencia:
                @isset($license_url)
                    <a href="{{ $license_url }}" target="_blank" rel="noopener noreferrer license external" class="link-template">
                @endisset
